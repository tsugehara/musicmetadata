  ...
  strtok.parse(stream, function (v, cb) {
    ...
    switch (cb.state) {
      ...
      case 2: // audio frame header
        ...
        if (readDuration && fileSize && frameCount === 3 && areAllSame(bitrates)) {
          fileSize(function (size) {
            ...
            callback('duration_milliseconds', Math.round(size / kbps * 1000))
            cb(done())
          })
          return strtok.DEFER
        }
        ...
        if (readDuration && frameCount === 4) {
          stream.once('end', function () {
            ...
            callback('duration_milliseconds', calcDurationMilliseconds(frameCount,
              header.samples_per_frame, header.sample_rate))
            done()
          })
        }
      ...
      case 4: // xtra / info header
        ...
        callback('duration_milliseconds', calcDurationMilliseconds(numFrames, ah.samples_per_frame, ah.sample_rate))
        return done()
...
function calcDurationMilliseconds (numFrames, samplesPerFrame, sampleRate) {
  return Math.round(numFrames * (samplesPerFrame / sampleRate) * 1000)
}
